...
```{r}
group_col <- c("#30113BFF", "#28BBECFF", "#94e637", "#FB8022FF", "#8A0000CF", "gray", "gray50", "black")
names(group_col) <- c("ER", "OB", "C", "AG", "D", "admixed", "VN present (1970-2000)", "VN future (2041-2060)")

group_col_af <- c("#30113BFF", "#28BBECFF", "#A2FC3CFF", "#FB8022FF", "#8A0000CF", "gray")
...
af_clim_file <- "/shared/projects/most_kmer/afterkiss/bioclim/updated_bioclim_AF.csv"
# af_clim_file <- "../bioclim/climatic_variables_af_chelsa.csv"
allaf_clim <- read.csv(af_clim_file)
allaf_clim <- allaf_clim[, !colnames(allaf_clim) %in% c("bio_10", "bio_1", "bio_3", "bio_14", "bio_9", "bio_16")]
# af_clim <- af_clim %>% 
#   # select(1:20) %>% 
#   column_to_rownames("Label") %>% 
#   dplyr::rename(group = snp_group) %>% 
...
```{r}
afclim_pca <- prcomp(af_clim[, grepl("bio", colnames(af_clim))], scale. = T)
summary(afclim_pca)
...
afprs <- autoplot(afclim_pca, x = 1, y = 2,
...
sum(eig[1:7,1]/sum(eig[,1]))
## contribution of temp vars (bio1-bio11) on PC1
sum(eig[1:7,2]/sum(eig[,2]))
```

```{r}
all_clim <- bind_rows(af_clim %>% 
                        dplyr::select(starts_with("bio"), group) %>%
                        mutate(province = "Africa"),
                  rob_ch_clim %>% 
                    dplyr::select(grep("bio", colnames(af_clim), value = T), 
                                  group, province, district))

vnclim_sc <- scale(rob_ch_clim[,grep("bio", colnames(af_clim), value = T)], center = afclim_pca$center)
...
climproj_mp <- cbind(climproj$x,
  ...
  summarise(across(PC1:PC13, mean))
...
predict_yield <- function(future_file, all_clim, climproj, yield_lm, current_yield, map) {
  ...
  bc_future_vn <- bc_future_vn %>% 
    ...
    filter(!province %in% c("Lâm Đồng", "Bình Định", "Bình Phước", "Khánh Hòa")) %>% 
    dplyr::rename(group = snp_group) %>% 
    dplyr::select(grep("bio", colnames(all_clim), value = T), group, province, district)
  ...
  futclim_sc <- scale(bc_future_vn[,1:13], 
...
bc_future_vn <- bc_future_vn %>% 
  ...
  filter(!province %in% c("Lâm Đồng", "Bình Định", "Bình Phước", "Khánh Hòa")) %>% 
  dplyr::rename(group = snp_group) %>% 
  dplyr::select(grep("bio", colnames(allaf_clim), value = T), group, province, district) # %>%
...
plot_list(vn_prs12, future_yield_pred[[1]]$projection, future_yield_pred[[2]]$projection,
...
png("/shared/projects/most_kmer/paper_plot/predict_yield_all.png", 
...
plot_list(future_yield_pred[[1]]$yield, future_yield_pred[[1]]$map,
          ...
          ncol = 6, widths = c(3,1),
          labels = LETTERS[1:12])
ggarrange(future_yield_pred[[1]]$yield, future_yield_pred[[1]]$map,
          future_yield_pred[[3]]$yield, future_yield_pred[[3]]$map,
          future_yield_pred[[5]]$yield, future_yield_pred[[5]]$map, 
          future_yield_pred[[2]]$yield, future_yield_pred[[2]]$map,
          future_yield_pred[[4]]$yield, future_yield_pred[[4]]$map,
          future_yield_pred[[6]]$yield, future_yield_pred[[6]]$map,
          ncol = 6, nrow = 2, common.legend = T, widths = rep(c(3,1), 3),
          labels = LETTERS[1:12])
dev.off()

tiff("/shared/projects/most_kmer/paper_plot/climate_af_vn.tiff", 
     width = 15, height = 10, units = "in", res = 720)
png("/shared/projects/most_kmer/paper_plot/climate_af_vn.png", 
     width = 12, height = 5, units = "in", res = 720)
plot_list(afprs, 
          future_yield_pred[[2]]$projection + ggtitle("") + theme(legend.position = "right"),
          labels = LETTERS[1:2])
dev.off()
...
tiff("/shared/projects/most_kmer/paper_plot/mean_climate_all.tiff", 
...
vn_prs12_ld <- autoplot(afclim_pca, x = 1, y = 2,
         ...
         loadings.label.repel = T,
         loadings.label.colour = "gray20", frame = F) +
  theme_minimal() + 
  theme(axis.title = element_text(size = 14)) +
  scale_colour_manual(values = c(group_col[1:5], "Vietnam" = "gray50")) +
  scale_fill_manual(values = c(group_col[1:5], "Vietnam" = "gray50")) 
...
png("/shared/projects/most_kmer/paper_plot/project_climate_all.png", 
...
go_all <- read.csv("all_gap_rerun.csv")
go_all <- go_all %>% 
  mutate(model = gsub("_2041-2060", "", model)) %>% 
...
plot_list(go_map_list[[7]], go_map_list[[1]], go_map_list[[2]], 
          ggplot() + theme_void(), go_map_list[[3]], go_map_list[[4]],
          ggplot() + theme_void(), go_map_list[[5]], go_map_list[[6]],
          nrow = 3, byrow = T)
...
```{r}
png("/shared/projects/most_kmer/paper_plot/go_clim.png", 
...
ggplot() +
  ...
  geom_point(data = best_present, aes(x = Long, y = Lat, color = group), size = 1) +
  geom_point(data = best_present, aes(x = Long, y = Lat, color = group), size = 0.1) +
...
worst_plots <- lapply(unique(worst_geno$model), function(m) {
  ...
  pl <- ggplot() +
    geom_sf(data = dt_pol, fill = "white", color = "#77a37b", size = 0.3) +
    geom_sf(data = vn_pol, fill = NA, color = "#255429", linewidth = .6) + 
    geom_point(data = worst_model, aes(x = long, y = lat, color = group), size = 1) +
               # show.legend = (m == "present, 1970-2000")) +
    # geom_line(data = mig_present, 
    #           aes(x = Long, y = Lat, 
    #               color = group,
...
plot_list(best_plots[[7]] + ggtitle("Most suitable in present"), 
          best_plots[[2]] + ggtitle("Most suitable in 2041-2060"),
          worst_plots[[7]] + ggtitle("Least suitable in present"), 
...
fut_best <- best_geno %>% 
  ...
  group_by(ind, occurrence) %>% 
  mutate(count = n()) %>% 
  distinct(ind, occurrence, long, lat, district, province, group, count) %>% 
  group_by(occurrence) %>% 
  arrange(desc(count)) %>% 
  slice_head(n = 1) %>% 
...
fut_best_plot <- ggplot() +
  geom_sf(data = dt_pol, fill = "white", color = "#77a37b", size = 0.3) +
  geom_sf(data = vn_pol, fill = NA, color = "#255429", linewidth = .6) + 
  geom_point(data = fut_best, aes(x = long, y = lat, color = group), size = 1) +
             # show.legend = (m == "present, 1970-2000")) +
  # geom_line(data = mig_present, 
  #           aes(x = Long, y = Lat, 
  #               color = group,
...
fut_worst_plot <- ggplot() +
    geom_sf(data = dt_pol, fill = "white", color = "#77a37b", size = 0.3) +
    geom_sf(data = vn_pol, fill = NA, color = "#255429", linewidth = .6) + 
    geom_point(data = fut_worst, aes(x = long, y = lat, color = group), size = 1) +
               # show.legend = (m == "present, 1970-2000")) +
    # geom_line(data = mig_present, 
    #           aes(x = Long, y = Lat, 
    #               color = group,
...
er_offset <- go_all %>% 
  ...
  summarise(mean_offset = mean(offset)) %>% 
  left_join(dist_yield %>% filter(group == "ER"))

er_offset_prs <- er_offset %>% 
  filter(model == "present")

er_offset_fut <- er_offset %>% 
  filter(model != "present")

go_yield <- ggplot(er_offset, 
       aes(x = mean_offset, y = yield)) + 
  geom_point(color = group_col["ER"]) +
  geom_smooth(method = 'lm', formula = y~x, color = group_col["ER"]) + 
  theme_minimal() +
  theme(axis.title = element_text(size = 14)) +
...
cor.test(er_offset_prs$mean_offset, er_offset_prs$yield)
cor.test(er_offset_prs$mean_offset, er_offset_prs$euclidean_distance)

yg_rel <- lm(yield~mean_offset, er_offset_prs)
yg_pred <- predict.lm(yg_rel, 
           er_offset_fut %>% dplyr::select(mean_offset),
           interval = "confidence")


yg_pred <- cbind(er_offset_fut, yg_pred) %>% 
  mutate(predictor = "genetic offset")


yc_pred <- do.call(rbind, lapply(future_yield_pred, function(i) {
  m <- unique(i$climchange$scenario)
  yd <- i$yield_change %>% 
    dplyr::select(district, province, future_yield, lwr, upr) %>% 
    rename(fit = future_yield) %>% 
    mutate(model = m)
  yd
  # print(m)
}))
yc_pred <- left_join(er_offset_fut, yc_pred) %>% 
  mutate(predictor = "climate")

fut_yield <- rbind(yg_pred, yc_pred)

fut_yield <- fut_yield %>% 
  mutate(yield_change = fit - yield, 
         yield_change_lwr = lwr - yield,
         yield_change_upr = upr - yield) %>% 
  arrange(yield_change) %>% 
  mutate(district = factor(district, levels = unique(district)))

## find most affected districts
fut_yield %>% 
  filter(predictor == "climate") %>% 
  group_by(model) %>% 
  summarise(tot = sum(abs(yield_change)))

fut_yield %>% 
  filter(predictor != "climate") %>% 
  slice_min(yield_change)

png("/shared/projects/most_kmer/paper_plot/predict_yield_clim_go.png", 
     width = 20, height = 10, units = "in", res = 1200)
ggplot(fut_yield, aes(x = district, y = yield_change, group = predictor)) + 
  facet_wrap(~model, ncol = 1) + 
  geom_col(aes(fill = yield_change/yield*100, color = predictor), 
           width = 0.6, show.legend = T, position = position_dodge(width = 0.5)) + 
  geom_errorbar(aes(ymin = yield_change_lwr, ymax = yield_change_upr, color = predictor), 
  ...
  # ylim(c(-1.2, 1.5)) +
  # scale_y_continuous(limits = c(-1.2, 1.5), expand = c(0, 0)) +
  scale_color_manual(values = c("#3288d5", "black")) +
  scale_fill_gradientn(colors = c("#c9ab12", "#d4c266", "white", "#83c78a", "#4d9454", "#2d6b34", "#1a451f"),
                       limit = c(-50, 120),
                       guide = guide_colorbar(title = "% Change")) 
dev.off()

yield_district <- ggplot(fut_yield %>% filter(model == "CNRM-CM6-1, ssp585"), 
                         aes(x = district, y = yield_change, group = predictor)) + 
  geom_col(aes(fill = yield_change/yield*100, color = predictor), 
           width = 0.5, show.legend = F, position = position_dodge(width = 0.7)) + 
  geom_errorbar(aes(ymin = yield_change_lwr, ymax = yield_change_upr, color = predictor), 
                width = 0.3, position = position_dodge(width = 0.7)) +
  geom_point(aes(fill = yield_change/yield*100), size = 0, alpha = 0) +
  theme_minimal() + 
...
clim_pred_map <- ggplot(merge(dt_pol, 
             fut_yield %>% filter(model == "CNRM-CM6-1, ssp585", predictor == "climate"), 
             by.x = "VARNAME_2", by.y = "district")) +
  # facet_grid(cols = vars(predictor)) +
  geom_sf(data = vn_pol, fill = "gray", color = "#5588d5", linewidth = .6) +
  geom_sf(aes(fill = yield_change/yield*100), show.legend = F,
          color = "#3288d5") +
  # geom_point(data = rob_ch_clim, aes(x = Long, y = Lat), color = "black", alpha = .3, size = 1, shape = 16) +
  theme_void() +
  scale_fill_gradientn(colors = c("#c9ab12", "#d4c266", "white", "#83c78a", "#4d9454", "#2d6b34", "#1a451f"),
                       limit = c(-50, 120),
                       guide = guide_colorbar(title = "% Change")) + 
...
go_pred_map <- ggplot(merge(dt_pol, 
             fut_yield %>% filter(model == "CNRM-CM6-1, ssp585", predictor != "climate"), 
             by.x = "VARNAME_2", by.y = "district")) +
  # facet_grid(cols = vars(predictor)) +
  geom_sf(data = vn_pol, fill = "gray", color = "black", linewidth = .6) +
  geom_sf(aes(fill = yield_change/yield*100), show.legend = F,
          color = "gray40") +
  # geom_point(data = rob_ch_clim, aes(x = Long, y = Lat), color = "black", alpha = .3, size = 1, shape = 16) +
  theme_void() +
  scale_fill_gradientn(colors = c("#c9ab12", "#d4c266", "white", "#83c78a", "#4d9454", "#2d6b34", "#1a451f"),
                       limit = c(-50, 120),
                       guide = guide_colorbar(title = "% Change")) +
  # ggtitle("genetic offset") + 
  theme(title = element_text(hjust = 0.5, size = 14))

pred_map <- ggarrange(clim_pred_map, go_pred_map)
png("/shared/projects/most_kmer/paper_plot/predict_yield_clim_go.png", 
     width = 12, height = 4, units = "in", res = 1200)
ggarrange(yield_district, pred_map, common.legend = T, widths = c(0.7, 0.25),
          labels = LETTERS[1:2], legend = "right")
dev.off()
...
## significant kmers for each bio variable in  lfmm
```{r}
library(qvalue)
lfmm_files <- list.files("/shared/projects/most_kmer/afterkiss/gea/lfmm_rda/out", 
                         "lfmm_results.tsv",
                         recursive = T, full.names = T)

f <- 0.01

count_lfmm <- lapply(lfmm_files, function(lf) {
  print(which(lfmm_files == lf))
  r <- read.delim(lf)
  count <- sapply(colnames(r), function(b) {
    pv <- r[,b]
    if (length(pv) == 0) return(0)
    qv <- qvalue(pv, fdr.level = f, pfdr = T)
    s <- which(qv$qvalues < f)
    return(length(s))
    })
  return(count)
})
