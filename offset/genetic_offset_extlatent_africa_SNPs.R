...
snp_mat <- fread("/shared/projects/afrob_seq/wgs_snp/vcf_bgi_2021/gbp_output/filtered_vcf_by_chrom/out.012", data.table = F,
                 drop = 1)
rownames(snp_mat) <- readLines("/shared/projects/afrob_seq/wgs_snp/vcf_bgi_2021/gbp_output/filtered_vcf_by_chrom/out.012.indv")
rownames(snp_mat)[which(rownames(snp_mat) == "BUD15")] <- "BUD15_merged"
colnames(snp_mat) <- readLines("/shared/projects/afrob_seq/wgs_snp/vcf_bgi_2021/gbp_output/filtered_vcf_by_chrom/out.012.pos")
snp_mat <- snp_mat[rownames(snp_mat) %in% com_inds$Label,] ## remove uncommon inds
signif_mat <- as.matrix(snp_mat)
