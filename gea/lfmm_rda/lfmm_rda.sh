...
clim=/shared/projects/most_kmer/afterkiss/bioclim/updated_bioclim_AF.csv
# clim=$(dirname $dist_dir)/lfmm_explanatory_af_chelsa.csv
out=$dist_dir/$n

## run script

## run lfmm with all 13 variables and K = 5
Rscript $(dirname $dist_dir)/rerun_lfmm_rda.R -b $bed -o $out -n $kmer_list -c $clim -K 5 -r 2 -f 0.01
