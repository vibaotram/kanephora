...
```{r}
test_clim <- clim[, c(1,3,7,12, ncol(clim))]
svg("/data/projects/rob/data/kmers/gea/climate_correlation.svg")
pairs.panels(test_clim, hist.col = "royalblue", ellipses = F, pch = c(0:5)[as.factor(clim$snp_group)])
...
```{r}
test_clim$snp_group <- factor(test_clim$snp_group, levels = c("ER", "OB", "C", "AG", "D", "hb"))
ggpairs(test_clim, columns = 1:4, aes(color = snp_group)) + 
...
ggsave("gea/climate_correlation_groups_gg.svg", width = 8, height = 8)
```
...
```{r}
suball_lfmm <- lfmm_ridge(Y = submat1, X = clim[, 1:19], K = 5)
...
-> 507468 significant k-mers
-> the histogram seems fine but half of the k-mers are significant 

### lfmm with only annual climatic variables

```{r}
subann_lfmm <- lfmm_ridge(Y = submat1, X = clim[, c(1,3,7,12)], K = 5)
...
subpca_lfmm <- lfmm_ridge(Y = submat1, X = clim_PC1, K = 5)
...
subpca2_lfmm <- lfmm_ridge(Y = submat1, X = clim_PC2, K = 5)
