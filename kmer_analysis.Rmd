...
```{r}
clim_file <- "./gea/climatic_variables.csv"
...
```{r}
clim_pca2 <- prcomp(clim[,1:20], scale = T)
summary(clim_pca2)
screeplot(clim_pca2)
library(ggfortify)
clim$snp_group <- factor(clim$snp_group, levels = c("hb","ER", "OB", "C", "AG", "D"))
...
autoplot(clim_pca2, x = 1, y = 2,
  ...
  theme_minimal() + 
  scale_colour_manual(values = c("gray", turbo(5))) +
  scale_fill_manual(values = c("gray", turbo(5))) 
...
cand_kmers <- rowSums(do.call(cbind, lapply(1:n, function(i) {
  df <- fread(log_files[i], skip = 10, nrows = 19, sep = " ", 
